...
from application.forms import UserLoginForm, AdminLoginForm, RegistrationForm
...
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        if not current_user.is_authenticated or current_user.is_admin == 0:
            abort(403)  # Forbidden error for non-admin users
...
def register_approval():
    ...
    if request.method == 'POST':
        ...
        for user in unapproved_users:
            ...
            flash('User registrations approved!', 'success')
        return redirect(url_for('routes.admin_dashboard'))
